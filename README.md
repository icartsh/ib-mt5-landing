...
**브로커에 관한 숫자는 페이지에서 만들지 않는다.** 전부 `docs/mim-broker-facts.md` 에서 가져오고,
그 문서에 없으면 페이지에 쓰지 않는다. 문구 가드가 "확인일 YYYY-MM-DD" 표기 여부를 검사한다.

### 아직 안 채워진 값 두 개

`public/config.js` 의 `mimSignupUrl`(우리 IB 코드가 붙은 가입 링크)과 `telegramUrl`.
**둘 다 비어 있으면 해당 버튼은 렌더링되지 않고**, 그 자리를 1:1 상담 신청 버튼이 대신한다.
죽은 링크나 "준비 중" 버튼을 내보내지 않기 위해서다. 값을 채우면 두 페이지의 버튼이 한꺼번에 살아난다.
