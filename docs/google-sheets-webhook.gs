 ...
 *
 * 이 스크립트를 고치더라도 성공했을 때 {"ok":true} 를 돌려주는 것은 유지해야 한다.
 * Apps Script 는 실패해도 HTTP 200 을 주기 때문에, 서버는 상태 코드가 아니라 이
 * 응답 본문으로 저장 성공을 판정한다(server/sinks.mjs). ok 가 없으면 서버는
 * 저장 실패로 보고 신청자에게 재시도를 안내한다 — 조용히 잃는 것보다 낫다.
 */
