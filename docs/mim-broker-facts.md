...
**지역 제한 (2026-09-03, 전 페이지 푸터 「지역 제한」 원문 확인)** — MIM 은 다음 관할권 거주자·시민을
대상으로 하지 않는다: **미국, 캐나다, 영국, 유럽연합(EEA), 이란, 북한, 미얀마, 방글라데시, 러시아, 벨라루스**
(원문 "포함하되 이에 국한되지 않는"). FATF 고위험·강화모니터링 대상국, UN·EU·OFAC 제재 목록 대상도 제외.

> **한국은 이 목록에 없다.** 8절의 "한국 거주자 대상 계좌 개설 제약" 미확인 항목이 여기까지는 풀렸다.
> 다만 원문이 **"이에 국한되지 않는"** 이라고 열어 두었으므로 `누구나 가입 가능` 류의 단정은 여전히 쓰지 않는다.
...
## 1-A. ⚠️ MIM 은 장내 선물 브로커가 아니다 — **Forex·CFD 플랫폼**이다 (2026-09-03 확인)

이 대장에서 **가장 자주 틀리는 자리**다. 우리 콘텐츠 전체가 `해외선물` 키워드로 서 있어서
상품까지 장내 선물이라고 착각하기 쉽다. **아니다.**

| 근거 | 확인 내용 |
|---|---|
| `sitemap.xml` 의 상품 페이지 전부 | `/kr/forex` `/kr/indices` `/kr/commodities` `/kr/crypto` `/kr/stocks` |
| 선물 상품 페이지 | `/kr/futures` `/kr/derivatives` `/kr/cme` → **전부 404.** 상품 라인 자체가 없다 |
| 전 페이지 푸터 | **"CFD 트레이딩의 미래."** |
| 홈 메타 설명 | "글로벌 **Forex·CFD** 플랫폼" |
| `/kr/indices` 나스닥100 심볼 칩 | **`NAS100`** — `NQ`·`MNQ` 는 사이트 어디에도 없다 |
| `/kr/stocks` | "**주식 CFD**를 통해 실물 주식을 보유하지 않고" |

**실무 규칙 — 원고·페이지·숏폼에 그대로 적용한다.**

- `해외선물` 을 **검색어로 쓰는 것은 막지 않는다.** 벤치마크(moneyinvest.co.kr)도 `해외선물` 을
  11회 쓰면서 상품 층위에서는 "400여 종 **CFD**" 라고 그대로 적는다. 문은 키워드로 열고,
  **상품은 정확한 이름으로 부른다.** 이 둘을 같이 하는 것이 정답이다.
- **CME 선물 심볼(NQ·MNQ·ES·MES…)과 계약 명세를 우리 계좌 안내에 쓰지 않는다.**
  독자가 MT5 주문 화면에서 **그 심볼을 찾지 못한다.** 나스닥100은 `NAS100` 이다.
- **만기·롤오버·근월물·거래소/청산 수수료는 CFD 에 없는 개념이다.** 체크리스트에 넣으면
  독자가 확인할 수 없는 항목을 확인하라고 시키는 셈이 된다.
- 비용 구조는 **스프레드(+계좌에 따라 커미션) + 스왑**이다. `/kr/indices` 는 지수에 대해
  "투명한 **제로 수수료** 환경"이라고 적는다 — 계약당·거래소·청산 층이 존재하지 않는다.
- **세금 분류가 갈린다.** 해외 장내파생상품(선물)의 파생상품 양도소득세 규정을
  CFD 손익에 그대로 옮겨 쓰면 안 된다. 확정 전까지 원고에 단정해 쓰지 않는다(8절 참조).

> 이 규칙은 `scripts/content-guard.mjs` 5번 「상품 정합성」 검사가 기계로 막는다.
> 2026-09-03 이전 원고 5편은 전부 이 검사를 **통과하지 못한다** — 그 상태로 PASS 였다.
...
- 서버 시간: **GMT+0 고정, 서머타임 없음** — 한국(KST)은 **+9시간**. 차트 시간 해석에 주의
  > ⚠️ **2026-09-03 정정.** 이 줄은 원래 "GMT+2 / GMT+3 (서머타임 적용 시)" 였다. **틀렸다.**
  > MIM `/kr/indices`·`/kr/stocks` 는 "연중 **고정된 GMT+0 서버 시간**을 채택" 하며
  > "계절적 시간 조정의 영향을 배제" 한다고 명시한다. `/signup` 안내에 이 값을 쓸 때 주의한다.
- 세션 시작·종료 시 **3분간 체결 버퍼**, **매일 1시간 정기 점검** (공식 표기)
...
## 4. 거래 상품 (확인됨) — **전부 CFD·FX 다. 장내 선물이 아니다** (1-A절)

> ⚠️ **심볼 표기 주의 (2026-09-03).** 아래 "대표 종목" 은 **분류를 설명하는 이름**이지
> MT5 주문 화면의 심볼이 아니다. MIM `/kr/indices` 공식 페이지가 화면에 띄우는 심볼 칩은
> **`US500` · `NAS100` · `US30` · `DE40`** 이다. 아래 표의 `US100` · `GER40` 과 다르다.
> **원고·페이지에 심볼을 쓰기 전에 실제 Market Watch 목록으로 확정한다** (8절 미확인 항목).
...
| **텔레그램 문의** | `https://t.me/icartsh_ib_bot` (표시명 `icartsh_IB`) | 고객 문의 **전용** 봇. `/api/telegram` webhook 이 받아서 사장님 대화로 옮긴다 → `docs/telegram-inquiry.md` |
| **리드 알림** | `@icartsh_answer_bot` | 서버가 사장님에게 보내는 **내부 채널**. 이 주소는 **어디에도 공개하지 않는다** — 아래 참고 |

> **왜 봇을 두 개로 나눴나 (2026-09-03)**
> 처음에는 `@icartsh_answer_bot` 하나를 문의 버튼에도 걸어 두었다. 그 봇은 원래
> **서버 → 사장님** 방향(리드 알림)으로만 쓰던 것이라 반대 방향은 아무도 읽지 않았고,
> 더 나쁘게는 알림 목적지를 "봇의 최근 대화"에서 자동으로 골랐다 —
> **고객이 그 봇에 말을 거는 순간 다음 리드 알림(이름 + 전체 전화번호)이 그 고객에게 갈 수 있었다.**
> 구글 시트가 꺼져 있어 텔레그램이 유일한 기록이므로, 사장님은 그 리드가 있었다는 것조차 몰랐을 것이다.
> 지금은 (1) 봇을 분리하고 (2) 대화가 둘 이상이면 아예 보내지 않도록 막아 두었다.
...
| **MT5 Market Watch 실제 심볼 목록** | 원고가 독자에게 "이 심볼을 찾으세요"라고 시키는 근거 | **심볼을 원고에 쓰지 않는다.** "본인 주문 화면에 표시되는 이름으로 확인" 으로 대체 (B5 방식). 확보 시도는 IB-26 |
| **CFD 손익의 국내 과세 분류** | B3(세금)이 통째로 이 위에 서 있다 | 해외 장내파생상품 규정을 CFD 에 그대로 옮겨 쓰지 않는다. **확정 전까지 B3 발행 보류** |
| MIM 파트너 계약서 원문의 마케팅 조항 | 이름·조건 표기 허용 범위의 최종 근거 | 지금은 **보수적으로** 간다 — 이름은 쓰되 조건 수치는 싣지 않음 (7절 결정) |
| ~~한국 거주자 대상 계좌 개설 제약~~ | 가입 안내의 전제 | **2026-09-03 일부 해소** — 공식 지역 제한 목록에 한국 없음(1절). 목록이 "이에 국한되지 않는"이라 `누구나 가능` 류 단정은 계속 금지 |
