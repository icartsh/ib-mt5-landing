...
const BANNED = [
  ...
  /* 거래량 단위에 붙는 지급·절감 약속 (IB-8 §2).
     브로커가 공시한 비용 사양 자체는 아래 5번(브로커 조건 수치)에서 따로 막는다. */
  [
...
const BROKER_FIGURE = /GB24203684|대표 종목|Segregated|분리 보관/;
const HAS_ASOF = /확인일[^\n]{0,12}\d{4}-\d{2}-\d{2}/;

for (const f of pages) {
  const t = text.get(f);
  if (BROKER_FIGURE.test(t) && !HAS_ASOF.test(t)) {
    fail(`[출처] ${f}: 브로커 사실을 쓰면서 "확인일 YYYY-MM-DD" 표기가 없다 (IB-6 체크리스트 5).`);
  }
}

/* ---------- 4-B. 브로커 조건 수치는 페이지에 옮겨 적지 않는다 ---------- */

/* IB-10 사장님 결정(2026-09-03, 질문 카드 3번 = "name_only"):
   **브로커명 표기는 가능하되, 계좌별 조건표(숫자)는 우리 페이지에 싣지 않는다.**

   두 가지 이유가 같은 방향을 가리킨다.
   ① 파트너 계약·마케팅 가이드라인 위반은 IB 코드 정지로 이어진다. 코드가 정지되면
      트래픽이 아무리 늘어도 수수료가 0이므로, 페이지 전체가 무의미해진다.
   ② 브로커 조건은 예고 없이 바뀐다. 우리가 옮겨 적은 숫자는 바뀐 뒤에도 그대로 남아
      결국 틀린 정보가 된다 — 우리 잘못으로.

   숫자는 docs/mim-broker-facts.md(내부 대장)에만 두고, 페이지는 MIM 공식 페이지로 보낸다.
   사람이 기억하는 대신 여기서 막는다 — 카피는 나중에 다른 사람이 손대기 때문이다. */
const BROKER_TERMS = [
  [/\b1\s*:\s*\d{2,4}\b/, "레버리지 배율"],
  [/\d\s*(?:\.\d+)?\s*핍/, "스프레드 핍 수치"],
  [/\$\s?\d/, "달러 금액(최소 입금·커미션)"],
];

for (const f of pages) {
  for (const [re, what] of BROKER_TERMS) {
    const hit = text.get(f).match(re);
    if (hit) {
      fail(
        `[브로커조건] ${f}: "${hit[0].trim()}" — ${what}를 페이지에 옮겨 적지 않는다. ` +
          `MIM 공식 페이지로 링크할 것 (IB-10 사장님 결정: 브로커명은 되나 조건표는 뺀다).`
      );
    }
  }
}
...
/* 우리 수익이 걸린 유일한 링크다. IB 코드가 빠진 주소도 **가입은 정상적으로 된다** —
   화면에도, 로그에도 아무 차이가 없고, 몇 달 뒤 정산이 비어 있을 때에야 드러난다.
   그래서 눈으로 보는 대신 여기서 본다. 코드가 바뀌면 이 검사가 먼저 깨지는 게 맞다. */
{
  const cfg = source.get("config.js") || "";
  const mim = (cfg.match(/mimSignupUrl:\s*"([^"]*)"/) || [])[1] ?? "";
  const tg = (cfg.match(/telegramUrl:\s*"([^"]*)"/) || [])[1] ?? "";

  if (mim && !/client\.myinvestmentmarkets\.com\/#\/[a-z]+\/[A-Za-z0-9]{4,}/.test(mim)) {
    fail(
      `[가입링크] config.js mimSignupUrl 에 IB 코드 구간이 없다 ("${mim}") — ` +
        `코드 없는 링크는 가입은 되지만 우리 실적으로 집계되지 않는다.`
    );
  }
